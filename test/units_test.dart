...
void main() {
  ...
  test("printHrMinLexical", () {
    expect(printHrMinLexical(const Duration(hours: 2, minutes: 10)),
        "2 hours 10 minutes");
    expect(printHrMinLexical(const Duration(hours: 1, minutes: 10)),
        "1 hour 10 minutes");
    expect(
        printHrMinLexical(const Duration(hours: 0, minutes: 10)), "10 minutes");
    expect(
        printHrMinLexical(const Duration(hours: 0, minutes: 0)), "0 minutes");
    expect(printHrMinLexical(const Duration(hours: 0, minutes: 1)), "1 minute");
    expect(
        printHrMinLexical(const Duration(hours: 0, minutes: 0)), "0 minutes");
    expect(printHrMinLexical(const Duration(hours: 1, minutes: 0)),
        "1 hour 0 minutes");
    expect(printHrMinLexical(const Duration(hours: 0, minutes: 90)),
        "1 hour 30 minutes");
    expect(printHrMinLexical(const Duration(hours: 5, minutes: 90)),
        "6 hours 30 minutes");

    expect(printHrMinLexical(const Duration(days: 1, hours: 5, minutes: 90)),
        "30 hours 30 minutes");
  });
  ...
  test("printValue", () {
    // Positive numbers
    expect(
        printDouble(value: 1.0, digits: 2, decimals: 1, autoDecimalThresh: 1.0),
        "1.0");
    expect(
        printDouble(
            value: 0.99, digits: 2, decimals: 1, autoDecimalThresh: 1.0),
        "0.99");
    ...
    expect(
        printDouble(
            value: -1.0, digits: 2, decimals: 1, autoDecimalThresh: 1.0),
        "-1.0");
    expect(
        printDouble(
            value: -0.99, digits: 2, decimals: 1, autoDecimalThresh: 1.0),
  ...
  test("richValue", () {
    ...
    expect(
        richValue(UnitType.speed, 1.0, decimals: 2)
            .children!
    ...
    expect(
        richValue(UnitType.vario, 1.0, decimals: 2)
            .children!
            .map((InlineSpan e) => e.toPlainText())
            .toList(),
        ["1.000", "m/s"]);
    ...
    expect(
        richValue(UnitType.distFine, 1.0, decimals: 2)
            .children!
            .map((InlineSpan e) => e.toPlainText())
            .toList(),
        ["1.00", "m"]);
    expect(
        richValue(UnitType.distCoarse, 1000.0, decimals: 2)
            .children!
            .map((InlineSpan e) => e.toPlainText())
            .toList(),
        ["1.00", "km"]);
    ...
    expect(
        richValue(UnitType.fuel, 1.0, decimals: 2)
            .children!
            .map((InlineSpan e) => e.toPlainText())
            .toList(),
        ["1.00", "L"]);
